...
| Asset grants last the whole session      | Playing a track lets the webview read that one file, and the grant is not taken back when the track leaves the library. Tauri can forbid a path, but a forbidden path outranks every later grant and cannot be undone, so revoking one would leave the file unplayable for the rest of the session if it were imported again. Serving audio through a protocol handler that checks each request would remove the trade-off, and is the way out if it ever matters. |
